# Copyright 2026.
"""Unified depth-memory attention variants on top of nanodo.

The variants share the NanoDO decoder surface while differing only in how
hidden information is carried across depth:

  - "baseline": identical behavior to `nanodo.model.TransformerDo`.
  - "dca": Deep Cross Attention. Stores hidden states across depth and reads
    them with GRN-v3 readers for Q/K/V plus a final GRN pool.
  - "moda": Mixture-of-Depths Attention. Stores projected depth K/V slots from
    previous layers and combines them with current-layer causal sequence K/V in
    one softmax. `moda_impl` selects a full naive reference or chunk-visible
    depth retrieval.
  - "mhc": Manifold Hyper-Connections. Maintains parallel residual streams,
    aggregates them with H_pre, and writes back with H_post plus either a
    Sinkhorn H_res or an identity H_res.
"""
...
class DepthMemConfig(base_model.DoConfig):
  ...
  # DCA input/output capacity ablations. Both default to disabled and only
  # support variant="dca".
  #
  # ple_layer: number of early DCA blocks that receive per-layer embeddings.
  #   0 disables PLE. The first cut fixes PLE dim to D, so a D=128 model with
  #   ple_layer=3 has V*(128 + 3*128) token-side parameters.
  # head_concat_layers: number of final DCA block outputs to concatenate before
  #   the LM head. 0 preserves the existing DCA readout (hidden-DCA final_grn or
  #   post-proj last hidden) with the tied embedding head. Nonzero uses an
  #   untied Dense head over [B, L, head_concat_layers * D].
  ple_layer: int = 0
  head_concat_layers: int = 0
...
def _validate_dca_edge_capacity_cfg(cfg):
  """Validate DCA-only PLE/readout ablation knobs."""
  assert cfg.ple_layer >= 0, "ple_layer must be >= 0"
  assert cfg.head_concat_layers >= 0, "head_concat_layers must be >= 0"
  assert cfg.ple_layer <= cfg.N, "ple_layer must be <= N"
  assert cfg.head_concat_layers <= cfg.N, "head_concat_layers must be <= N"
  if cfg.variant != "dca" and (
      cfg.ple_layer != 0 or cfg.head_concat_layers != 0
  ):
    raise NotImplementedError(
        "ple_layer/head_concat_layers are only supported for variant='dca'"
    )
...
def _flash_topk_index_buffer(q_BLHD, keys_BNHD, kp_N, q_pos_L, compare, k, block):
  """Streaming top-k logits plus flattened key indices for one key set."""
  assert compare in ("causal", "strict"), (
      f"compare must be 'causal' or 'strict', got {compare!r}")
  B, L, H, Dh = q_BLHD.shape
  N = keys_BNHD.shape[1]
  ...
  nt = kt_all.shape[0]
  k = min(int(k), N)
  idx_t = jnp.arange(nt * block, dtype=jnp.int32).reshape(nt, block)
  buf0 = jnp.full((B, H, L, k), neg, jnp.float32)
  idx0 = jnp.full((B, H, L, k), -1, jnp.int32)

  def body(carry, tile):
    buf, idx = carry
    kt, kpt, kv, key_idx = tile
    ...
    cmask = cmask & kv[None, :]
    s = jnp.where(cmask[None, None], s, neg)
    tile_idx = jnp.where(cmask, key_idx[None, :], -1)
    tile_idx = jnp.broadcast_to(tile_idx[None, None], (B, H, L, block))
    cand = jnp.concatenate([buf, s], axis=-1)
    cand_idx = jnp.concatenate([idx, tile_idx], axis=-1)
    buf, pos = jax.lax.top_k(cand, k)
    idx = jnp.take_along_axis(cand_idx, pos, axis=-1)
    return (buf, idx), None

  (buf, idx), _ = jax.lax.scan(
      jax.checkpoint(body), (buf0, idx0), (kt_all, kp_t, kvalid, idx_t))
  return buf, idx


def _gather_topk_values(vals_BNHD, idx_BHLK):
  """Gather vals_BNHD at per-(B,H,L,K) flattened key indices."""
  vals_BHND = vals_BNHD.transpose(0, 2, 1, 3)
  safe_idx = jnp.maximum(idx_BHLK, 0)

  def gather_b(v_HND, i_HLK):
    return jax.vmap(lambda v_ND, i_LK: v_ND[i_LK], in_axes=(0, 0))(
        v_HND, i_HLK)

  return jax.vmap(gather_b, in_axes=(0, 0))(vals_BHND, safe_idx)


def _topk_list_stream(logits_BHLK, idx_BHLK, vals_BNHD, block):
  """Convert a selected top-k list into an online-softmax stream tuple."""
  B, H, L, K = logits_BHLK.shape
  Dh = vals_BNHD.shape[-1]
  neg = jnp.finfo(jnp.float32).min
  block = min(max(int(block), 1), K)
  nt = -(-K // block)
  n = nt * block
  pad = n - K
  logits = jnp.pad(logits_BHLK, ((0, 0), (0, 0), (0, 0), (0, pad)),
                   constant_values=neg)
  idx = jnp.pad(idx_BHLK, ((0, 0), (0, 0), (0, 0), (0, pad)),
                constant_values=-1)
  logits = logits.reshape(B, H, L, nt, block).transpose(3, 0, 1, 2, 4)
  idx = idx.reshape(B, H, L, nt, block).transpose(3, 0, 1, 2, 4)
  m0 = jnp.full((B, H, L, 1), neg, jnp.float32)
  l0 = jnp.zeros((B, H, L, 1), jnp.float32)
  acc0 = jnp.zeros((B, H, L, Dh), jnp.float32)
  ...
  def body(carry, tile):
    ...
    logit, key_idx = tile
    valid = key_idx >= 0
    selected_v = _gather_topk_values(vals_BNHD, key_idx).astype(jnp.float32)
    masked = jnp.where(valid, logit, neg)
    m2 = jnp.maximum(m, jnp.max(masked, axis=-1, keepdims=True))
    corr = jnp.exp(m - m2)
    safe_logits = jnp.where(valid, logit, m2)
    p = jnp.where(valid, jnp.exp(safe_logits - m2), 0.0)
    l = l * corr + jnp.sum(p, axis=-1, keepdims=True)
    acc = acc * corr + jnp.einsum("bhlk,bhlkd->bhld", p, selected_v)
    return (m2, l, acc), None

  (m, l, acc), _ = jax.lax.scan(
      jax.checkpoint(body), (m0, l0, acc0), (logits, idx))
  return m, l, acc


def _flash_topk_stream(q_BLHD, keys_BNHD, vals_BNHD, kp_N, q_pos_L, compare, k,
                       block):
  """Streaming top-k attention stream without rescanning the full key set."""
  logits, idx = _flash_topk_index_buffer(
      q_BLHD, keys_BNHD, kp_N, q_pos_L, compare, k, block)
  return _topk_list_stream(logits, idx, vals_BNHD, block)
...
def _moda_attention_flash(q, k_cur, v_cur, depth_k, depth_v, cfg, gate):
  ...
  if cfg.moda_two_stream:
    ...
    if sel == "topk":
      m_b, l_b, acc_b = _flash_topk_stream(
          q, dk, dv, depth_time, q_pos, "strict", cfg.moda_rest_topk, block)
    else:  # all
  ...
  if sel == "all":
    ...
    out = _merge([
        row(),
        _flash_col(q, depth_k, depth_v),
        _flash_topk_stream(q, dk, dv, depth_time, q_pos, "strict", k, block),
    ])
...
class DepthMemTBlock(nn.Module):
  ...
  def __call__(
      ...
      x_BxLxD: jax.Array,
      memory,
      ple_input_BxLxD: jax.Array | None = None,
  ):
    cfg = self.cfg
    if cfg.variant == "dca":
      if _dca_uses_post_proj(cfg):
        return self._call_dca_post_proj(x_BxLxD, memory, ple_input_BxLxD)
      return self._call_dca(x_BxLxD, memory, ple_input_BxLxD)
    if cfg.variant == "mhc":
  ...
  def _apply_ple(
      self,
      block_out_BxLxD: jax.Array,
      ple_input_BxLxD: jax.Array | None,
  ) -> jax.Array:
    """Gate a per-layer embedding into a DCA block output."""
    if ple_input_BxLxD is None:
      return block_out_BxLxD
    cfg = self.cfg
    gate = nn.Dense(
        cfg.D,
        name="ple_gate",
    ...
    delta = jax.nn.gelu(gate) * ple_input_BxLxD
    delta = nn.Dense(
        cfg.D,
        name="ple_proj",
        kernel_init=fsdp.init("mlp_kernel", cfg),
    ...
    delta = nn.RMSNorm(dtype=cfg.dtype, name="ple_norm")(delta)
    return block_out_BxLxD + delta

  def _call_dca(self, x_BxLxD, memory, ple_input_BxLxD):
    cfg = self.cfg
    ...
    block_out = self._apply_ple(block_out, ple_input_BxLxD)
  ...
  def _call_dca_post_proj(self, x_BxLxD, memory, ple_input_BxLxD):
...
def _concat_depth_states(states_KxBxLxD: jax.Array) -> jax.Array:
  """Move a static depth stack into the feature axis: [K,B,L,D] -> [B,L,K*D]."""
  K, B, L, D = states_KxBxLxD.shape
  return jnp.moveaxis(states_KxBxLxD, 0, 2).reshape(B, L, K * D)
...
class DepthMemTransformerDo(nn.Module):
  ...
  def setup(self):
    ...
    _validate_dca_edge_capacity_cfg(cfg)
    ...
    if cfg.ple_layer > 0:
      self.ple_embed = nn.Embed(
          num_embeddings=cfg.V,
          features=cfg.ple_layer * cfg.D,
          embedding_init=fsdp.init("embedding", cfg),
      )
      self.ple_model_projection = nn.DenseGeneral(
          features=(cfg.ple_layer, cfg.D),
          axis=-1,
          kernel_init=fsdp.init("attn_in_proj", cfg),
          use_bias=False,
      ...
      self.ple_projection_norm = nn.RMSNorm(dtype=cfg.dtype)

    if cfg.head_concat_layers > 0:
      self.head_ln = nn.RMSNorm(dtype=cfg.dtype)
      self.head = nn.Dense(
          cfg.V,
  ...
  def _encode_ple_inputs(
      self,
      y_BxL: jax.Array,
      embedding_BxLxD: jax.Array,
  ) -> jax.Array | None:
    """Return [B,L,ple_layer,D] PLE inputs, or None when disabled."""
    cfg = self.docfg
    if cfg.ple_layer == 0:
      return None
    B, L = y_BxL.shape
    token_ple = self.ple_embed(y_BxL).reshape(B, L, cfg.ple_layer, cfg.D)
    model_ple = self.ple_model_projection(embedding_BxLxD)
    model_ple = self.ple_projection_norm(model_ple)
    return (token_ple + model_ple) * jax.lax.rsqrt(2.0).astype(token_ple.dtype)

  def _dca_project_concat_head(self, states_KxBxLxD: jax.Array) -> jax.Array:
    readout = _concat_depth_states(states_KxBxLxD)
    readout = self.head_ln(readout)
    return self.head(readout.astype(jnp.float32))
  ...
  def __call__(self, y_BxL: jax.Array):
    ...
    ple_inputs = self._encode_ple_inputs(y_BxL, y_BxLxD)

    if cfg.variant == "dca" and not _dca_uses_post_proj(cfg):
      memory = jnp.zeros((cfg.N + 1, B, L, cfg.D), dtype=_cfg_dtype(cfg))
      memory = memory.at[0].set(y_BxLxD)
      for i, block in enumerate(self.blocks):
        ple_input = ple_inputs[:, :, i, :] if i < cfg.ple_layer else None
        _, memory = block(None, memory, ple_input)
      if cfg.head_concat_layers > 0:
        return self._dca_project_concat_head(
            memory[1:][-cfg.head_concat_layers :]
        )
    ...
    if cfg.variant == "dca" and _dca_uses_post_proj(cfg):
      ...
      for i, block in enumerate(self.blocks):
        ple_input = ple_inputs[:, :, i, :] if i < cfg.ple_layer else None
        last, memory = block(last, memory, ple_input)
        if i >= cfg.N - cfg.head_concat_layers:
          last_states.append(last)
      if cfg.head_concat_layers > 0:
        return self._dca_project_concat_head(jnp.stack(last_states, axis=0))
